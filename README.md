...
- ✅ **v2.1.2**: 修復 segSNR 計算 bug，更新基準測試結果
- ✅ **v2.1.1**: V3 系列 alpha_xi=0.96 同步
- ✅ 模塊化設計，易於理解和擴展
...
### 對標結果（v2.1.2 - 2026-01-06）

與業界標準 Speex/RNNoise 對比：

#### segSNR 改善排名（分段 SNR 提升，dB）- 主要指標

| 排名 | 方法 | segSNR 改善 | fwSegSNR | WSS | PESQ | STOI |
|------|------|-------------|----------|-----|------|------|
| 🥇 1 | **RNNoise** | **+4.72 dB** | 3.93 | 66.4 | 1.69 | **0.90** |
| 🥈 2 | **V4 (IMCRA-OMLSA)** | +4.60 dB | -7.32 | 96.5 | 1.19 | 0.20 |
| 🥉 3 | **V3 (SPP-MMSE)** | +2.65 dB | -4.92 | 99.3 | 1.52 | 0.17 |
| 4 | V3-2 (MMSE-LSA) | +2.56 dB | -4.81 | 106.6 | 1.43 | 0.14 |
| 5 | V2 (Wiener) | +2.54 dB | -4.71 | 109.0 | 1.27 | 0.14 |
| 6 | Speex | +2.40 dB | 6.14 | 69.9 | 1.44 | 0.88 |
| 7 | V3-4 (Laplacian) | +2.33 dB | -4.41 | 108.6 | 1.43 | 0.17 |
| 8 | V1 (Spectral Sub) | +2.11 dB | -4.30 | 111.0 | 1.44 | 0.13 |
| 9 | V3-3 (PMMSE) | +2.08 dB | -4.27 | 105.5 | 1.43 | 0.15 |

#### PESQ/STOI 排名（感知語音質量）- 參考指標

| 排名 | 方法 | PESQ | STOI | 特點 |
|------|------|------|------|------|
| 🥇 1 | **RNNoise** | 1.69 | **0.90** | **深度學習，STOI 最佳** |
| 🥈 2 | **Speex** | 1.44 | 0.88 | 傳統基準，平衡性能 |
| 🥉 3 | **V3 (SPP-MMSE)** | **1.52** | 0.17 | **傳統算法最佳 PESQ** |
| 4 | V1 (Spectral Sub) | 1.44 | 0.13 | 簡單高效 |
| 5 | V3-3 (PMMSE) | 1.43 | 0.15 | 感知優化 |
| 6 | V3-4 (Laplacian) | 1.43 | 0.17 | 平衡性能 |
| 7 | V3-2 (MMSE-LSA) | 1.43 | 0.14 | 對數域 MMSE |
| 8 | V2 (Wiener) | 1.27 | 0.14 | 基礎方法 |
| 9 | V4 (IMCRA-OMLSA) | 1.19 | 0.20 | 強降噪，但過度抑制 |

**⚠️ v2.1.2 關鍵發現**：
- ⚠️ **V4 存在過度抑制問題**：輸出能量僅保留 ~1% 的輸入能量
  - segSNR 看起來不錯 (+4.60 dB)，但 PESQ 最低 (1.19)
  - 語音被嚴重削弱，實際聽感很差
- ✅ **RNNoise 綜合最佳**：segSNR +4.72 dB，STOI 0.90
- ✅ **V3 傳統算法最佳推薦**：PESQ 1.52，segSNR +2.65 dB，平衡性能
- 🐛 **修復 segSNR 計算 bug**：之前錯誤地將 sample_rate 傳遞給 frame_size 參數
- 📊 詳細報告：[results/benchmark_comparison.md](results/benchmark_comparison.md)
...
### v2.1.2 (2026-01-06) ✨ 最新
- 🐛 **修復 segSNR 計算 bug**: benchmark_comparison.py 錯誤地將 sample_rate 傳遞給 frame_size 參數
  - 之前的排名數據不正確，V4 segSNR 從 +5.68 dB 修正為 +4.60 dB
- ⚠️ **發現 V4 過度抑制問題**: 輸出能量僅保留 ~1% 的輸入能量
  - 原因：OMLSA 增益計算中的幀間限制和時間平滑導致增益持續下降
  - 雖然 segSNR 數值看起來不錯，但實際語音品質很差 (PESQ 最低)
...
**推薦使用 V3（平衡性能，傳統算法最佳 PESQ）或 RNNoise（深度學習，綜合最佳）版本以獲得最佳降噪效果！**

**⚠️ V4 警告**：雖然 V4 的 segSNR 數值看起來不錯，但存在嚴重的過度抑制問題，輸出能量僅保留 ~1%，實際聽感很差。建議使用 V3 替代。
