...
### ⚪ v1.2.0 - 噪聲場景轉換適應機制（計劃中）

**目標**：讓 V1-V4 能夠檢測並適應噪聲類型突變（如從辦公室噪聲切換到街道噪聲）
...
- ⚪ 快速適應機制
  - V1: 噪聲重估機制（200-400ms）
  - V2/V3: 動態 alpha 切換（100-200ms，alpha: 0.95 → 0.5）
  - V4: IMCRA 快速追蹤模式（300-600ms，L: 150 → 50）

#### 預期效果
- 適應速度：0.5-2 秒（25-100 幀）
- 加速比：2-4 倍（相比原始適應時間）
- 誤檢率：< 1-2%
- 計算開銷：每幀增加 < 1%
