...
def _build_reference_denoiser(sample_rate, config_dir, mode='full'):
    """Build the production V3-2 MmseLsaDenoiser exactly like process_audio.py.

    mode='full' (default) → shipped V3-2; mode='stationary' → content-preservation
    preset (Wiener lower-bound + tonal-veto). The C side mirrors this via
    mmse_lsa_config_stationary(); pass the matching mode to parity_runner.
    """
    ...
    return create_denoiser_from_config('V3-2', config_dir, sample_rate, mode=mode)
...
def cmd_dump(args):
    ...
    denoiser = _build_reference_denoiser(sr, config_dir, mode=args.mode)
